...
def append_message(image_url = None, query = None, audio_transcript = None):
    message_list = []
    if image_url:
...
@cl.on_message                    #decorator by chainlit, gets called whenever a message is sent by the user
async def chat(msg: cl.Message):
    ...
    if len(images) == 0 and len(audios) == 0:
        response = append_message(query= msg.content)

    elif len(audios) == 0:
        response = append_message(image_url= image_url, query = msg.content)
    
    else:
        response = append_message(query= msg.content, audio_transcript= text)

    response_msg_content = response.message.content
